...
public class UserServiceImpl implements UserService {
    ...
    public boolean deleteUser(Long id) {
        ...
        if (byId.isPresent()) {
            ...
            for (AppClient client : all) {
                for (Hobby hobby : byId.get().getHobbyOffers()) {
                    client.getHobbyMatches().remove(hobby);
                    client.getSavedHobbies().remove(hobby);
                }
    ...
    public void findAndRemoveHobbyFromClientsRecords(Hobby hobby) {
        ...
        for (AppClient appClient : all) {
            appClient.getSavedHobbies().remove(hobby);
            appClient.getHobbyMatches().remove(hobby);
        }
    }
